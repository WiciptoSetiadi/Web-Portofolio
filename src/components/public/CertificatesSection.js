...
export class CertificatesSection {
  ...
  renderCertificateCard(cert, index) {
    ...
    if (!hasFile) {
      ...
      // PDF: Show an embedded page preview
      previewHtml = `
        <div class="w-full h-full relative overflow-hidden pointer-events-none" title="Click to view">
          <embed src="${cert.image_url}#toolbar=0&navpanes=0&scrollbar=0&view=FitH" type="application/pdf" class="w-full h-full" />
          <div class="absolute inset-0 bg-transparent"></div>
        </div>
        <div class="absolute top-2 right-2 bg-red-500 text-white text-[10px] font-bold px-2 py-0.5 rounded">PDF</div>
      `;
    } else {
      // Image: show directly
