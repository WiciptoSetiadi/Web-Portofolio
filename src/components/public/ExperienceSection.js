...
export class ExperienceSection {
  ...
  async render() {
    ...
    return `
      <section id="experience" class="py-24 bg-gray-50 dark:bg-slate-800 border-t border-gray-100 dark:border-slate-700">
  ...
  renderTimelineItem(exp, index, presentLabel) {
    ...
    const alignClass = isLeft ? 'flex-row-reverse left-timeline' : 'right-timeline';
    const textBaseAlign = isLeft ? 'text-right' : 'text-left';
    ...
    const docHtml = exp.doc_url ? (() => {
      ...
      if (isPdfDoc) {
        return `
          <a href="${exp.doc_url}" target="_blank" class="group/doc mt-4 flex items-center gap-3 p-3 rounded-xl bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 hover:border-primary-400 transition-colors ${isLeft ? 'flex-row-reverse' : ''}">
            <div class="w-10 h-10 rounded-lg bg-red-50 text-red-400 flex items-center justify-center flex-shrink-0">
              <i data-lucide="file-text" class="w-5 h-5"></i>
            </div>
            <div class="flex-1 min-w-0 ${textBaseAlign}">
    ...
    return `
      <!-- Timeline Item -->
      <div class="mb-8 flex justify-between items-center w-full ${alignClass}" data-aos="${aosAttr}">
        <div class="order-1 w-5/12"></div>
        <div class="z-20 flex items-center order-1 bg-primary-500 shadow-xl w-8 h-8 rounded-full">
          <h1 class="mx-auto font-semibold text-lg text-white"><i data-lucide="briefcase" class="w-4 h-4"></i></h1>
        </div>
        <div class="order-1 bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 p-6 w-5/12">
          
          <div class="flex items-center gap-4 mb-3 ${isLeft ? 'justify-end flex-row-reverse' : ''}">
            ${exp.logo_url
        ? (exp.logo_url.toLowerCase().endsWith('.pdf')
          ...
          : `<img src="${exp.logo_url}" alt="${exp.company}" class="w-10 h-10 rounded object-contain bg-white flex-shrink-0">`
        )
        : ''}
            <div class="${textBaseAlign}">
              <h3 class="font-bold text-gray-900 dark:text-white text-xl">${exp.position}</h3>
              <div class="text-primary-600 dark:text-primary-400 font-medium">${exp.company}</div>
            </div>
          </div>
          
          <div class="text-sm text-slate-500 mb-4 font-mono bg-gray-50 dark:bg-slate-800 inline-block px-2 py-1 rounded ${isLeft ? 'float-right' : 'float-left'}">
            ${startDate} - ${endDate}
          </div>
          ...
          ${exp.tags && exp.tags.length > 0 ? `
            <div class="flex flex-wrap gap-2 mt-4 ${isLeft ? 'justify-end' : 'justify-start'}">
              ${exp.tags.map(tag => `<span class="px-2 py-1 bg-gray-100 dark:bg-slate-800 text-slate-500 text-xs rounded">${tag}</span>`).join('')}
            </div>
          ` : ''}
